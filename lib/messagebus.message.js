...
MessageBusMessage.prototype.defineProperty = function({ name, value, isHeader = false }) {
    ...
    let newProperty = { Id: newId, name, value, isHeader };
    Object.defineProperty(this, newProperty.name, {
        get : () => newProperty.value,
        set : (value) => { 
            newProperty.value = value;
        }
    });
    let thisExtendedMessageProperties = this.getProperties({});
    thisExtendedMessageProperties = thisExtendedMessageProperties.filter(p => p.name !== newProperty.name);
    extendedMessages = extendedMessages.filter(p => p.Id !== newProperty.Id);
...
MessageBusMessage.prototype.update = function(message) {
    if (message instanceof MessageBusMessage) {
        for(const property of message.getProperties()) {
            this.extend({ propertyName: property.name, propertyValue: property.value, isHeader: property.isHeader});
        };
    } else {
