...
function MessageBusMessage({ Id, headers, body, status }) {
    this.Id = Id || utils.generateGUID();
    this.status = MessageBusMessageStatus[status] || MessageBusMessageStatus.None
    if (headers) {
...
MessageBusMessage.prototype.getProperties = function(headersOnly) {
    return extendedMessages.filter(extM => extM.Id === this.Id && (headersOnly !== undefined && extM.isHeader === headersOnly) );
};
