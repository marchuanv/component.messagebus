const { Communication, CommunicationIncomingMessage, CommunicationOutgoingMessage } = require("component.communication");
    ...
    communication.receive(async (incomingCommMessage) => {
        if (incomingCommMessage instanceof CommunicationIncomingMessage) {
            ...
            if (messageSubscribers.length > 0) {
                ...
                return new CommunicationOutgoingMessage( firstSuccesfulSubscriber || firstUnsuccesfulSubscriber );
            } else {
                return new CommunicationOutgoingMessage({
            ...
            throw new Error("incomingCommMessage parameter is not of type: CommunicationIncomingMessage");
        }
    });
};
...
MessageBus.prototype.publish = async function (message) {
    if (message instanceof MessageBusMessage) {
        try {
            ...
            const messageToSend = new CommunicationIncomingMessage(message);
            messageToSend.body.channel = message.channel;
            messageToSend.validate();
            const { statusCode, statusMessage } = await communication.send(messageToSend);
            if (statusCode < 200 && statusCode > 299) {
                throw new Error(`failed to publish message id ${message.id} to the ${message.channel} channel. Error: ${statusMessage}`);
            }
        } catch (err) {
            if (currentControlId) {
