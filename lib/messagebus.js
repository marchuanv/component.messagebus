...
function MessageBus() {
    ...
    communication.receive(async (incomingCommMessage) => {
        ...
        const messageSubscribers = this.subscribers.filter(s => s.Id === incomingMessageBusMessage.Id);
        if (messageSubscribers.length > 0) {
            if (!messageSubscribers.find(s => s.subscriptionId && s.subscriptionId === incomingMessageBusMessage.subscriptionId)) {
                ...
                return new CommunicationMessage(firstSuccesfulSubscriber);
            } else {
                incomingCommMessage.callback = async (data) => {
                    return await this.publish({ component: received.component, message: data });
                };
                await this.subscribe(incomingCommMessage);
            }
        } else {
            return new CommunicationMessage({ Id: incomingMessageBusMessage.Id, headers: {}, body: {}, statusCode: 404, statusMessage: `${incomingMessageBusMessage.Id} does not have any subscribers` });
        }
    });
};
