...
function MessageBus({ host, port, ishttp }) {
    this.host = host;
    this.port = port;
    this.communication = new Communication({ host, port, ishttp });
    this.communication.receive(async (incomingCommMessage) => {
        if (incomingCommMessage instanceof CommunicationIncomingMessage) {
            ...
            const messageSubscribers = subscribers.filter(s => s.channels.find( channel => channel === incomingMessageBusMessage.channel));
...
MessageBus.prototype.publish = async function (message) {
    if (message instanceof MessageBusMessage) {
        ...
        const receivedMessage = await this.communication.send(messageToSend);
