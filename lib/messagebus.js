...
function MessageBus() {
    ...
    communication.receive(async ({ headers, text }) => {
        ...
        const componentSubscribers = this.subscribers.find(s => s.component === received.component);
        if (received.issubscription) {
            if (!componentSubscribers.find(s => s.Id === received.message.subscriptionId)) {
                await this.subscribe({ component: received.component, callback: async (data) => {
                    await this.publish({ component: received.component, message: data });
                }, Id: received.message.subscriptionId });
            }
        }
        if (componentSubscribers) {
            const subscriberMessages = [];
            for(const componentSubscriber of componentSubscribers) {
                if (await componentSubscriber.validateCallback(received.message)) {
                    subscriberMessages.push(new Message(await componentSubscriber.callback(received.message)));
                }
            };
...
MessageBus.prototype.subscribe = async function ({ component, callback, callbackValidate })  {
    if (!component || !callback) {
        throw new Error("missing parameters: component OR callback");
    }
    callbackValidate = callbackValidate? callbackValidate : () => true;
    const newSubscription = { Id: utils.generateGUID(), callback, callbackValidate };
    const componentSubscribers = this.subscribers.find(s => s.component === component);
    if (componentSubscribers) {
        componentSubscribers.callbacks.push(newSubscription);
    } else {
        componentSubscribers.push({ component, callbacks: [newSubscription] });
    }
    ...
    return await communication.send({ data: messageToSend });
};

MessageBus.prototype.publish = async function ({ component, message }) {
    const messageToSend = new Message({ component, success: false, message });
