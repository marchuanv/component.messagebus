...
MessageBus.prototype.subscribe = async function ({ componentName, callback, validateCallback })  {
    ...
    const foundAtIndex = this.subscribers.findIndex(s => s.component === componentName);
    if (foundAtIndex > -1) {
        this.subscribers.splice(foundAtIndex, 1);
    }
    this.subscribers.push({ component: componentName, callback, validateCallback });
};

MessageBus.prototype.publish = function ({ componentName, message }) {
    const messageToSend = new Message({ component: componentName, success: false, message });
        ...
        if (matchCurrentControlId(controlId)) { //wait until control is released
            return setTimeout(async () => {
                await resolve(await module.exports.publish({
                    componentName: messageToSend.component,
